...
const list = async (req, res) => {
  try {
    ...
    const { count, rows } = await Bet.findAndCountAll({
      include: [{ model: Match, as: 'match', include: [{ model: Team, as: 'homeTeam' }, { model: Team, as: 'awayTeam' }] }, { model: BetTotal, as: 'total' }, { model: BetMoneyline, as: 'moneyline' }], offset: page - 1, limit: pageSize, order: [
